...
async def simulate_and_respond(simulator, batch_json, queue, estimate_block=False):
...
async def rcv_msg(
    ...
    async for jmsg in websocket:
        try:
            ...
            logger.debug(jmsg)
            task = asyncio.create_task(simulate_and_respond(simulator, solution, queue, req.estimate_block))
...
async def handler(websocket: WebSocketServerProtocol, path:str):
    ...
    try:
        tasks = [asyncio.create_task(rcv_msg(websocket, simulator, queue)),
...
async def main():
    async with websockets.server.serve(handler, "", 8001, ping_interval=10, ping_timeout=20):
